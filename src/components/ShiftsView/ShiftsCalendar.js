...
// Assign overlapping shift groups to visual rows (like calendar event lanes)
...
// Group same-time shifts together
...
const ROW_HEIGHT = 56; // px per shift row in a department lane
...
const ShiftsCalendar = ({ employee, onBack }) => {
  ...
  const axisBounds = (() => {
    ...
    return {
      startMin: Math.max(0,        Math.floor(lo / 60) * 60 - 60),
      endMin:   Math.min(48 * 60,  Math.ceil(hi  / 60) * 60 + 60),
    };
  })();
  ...
  const showMidnight = axisBounds.endMin > 1440;
          ...
          <div className="sv-timeline-inner">
            ...
            <div className="sv-time-axis">
              ...
              <div className="sv-axis-track">
                {hourMarkers.map(m => (
                  <span key={m} className="sv-hour-label" style={{ left: `${toPct(m)}%` }}>
                    {formatAxisMin(m)}
                  </span>
                ))}
                {showMidnight && (
                ...
                <div key={deptEntry.deptName} className="sv-dept-lane">
                  ...
                  <div className="sv-dept-track" style={{ height: laneH }}>
                    ...
                    {deptEntry.groups.map(g => {
                      ...
                      const widthPct  = ((eMin - sMin) / axisRange) * 100;
                      ...
                      return (
                        <div
                          ...
                          className={`sv-shift-block${isOwn ? ' sv-shift-own' : ''}`}
                          style={{
                            left:            `${leftPct}%`,
                            width:           `${widthPct}%`,
                            top:             `${topPx}px`,
      ...
      {popup && (
        <div className="sv-popup-overlay" onClick={() => setPopup(null)}>
          <div className="sv-popup-sheet" onClick={e => e.stopPropagation()}>
            ...
            <div className={`sv-popup-cards sv-popup-cards-${popup.groups.length}`}>
              {popup.groups.map(g => {
                  ...
                  >
                    ...
                    <div className="sv-card-chips">
                      {g.employees.map(emp => (
                        ...
                        >
                          <span className="sv-chip-initial"
            ...
            {popup.groups.length > 1 && (
              <div className="sv-popup-overlap-note">
                These shifts overlap on this day
              </div>
            )}
          </div>
        </div>
